#! python3
# messageBot.py - sends out a message to a selected group of Google Hangout contacts

import time, pyautogui

def auto_message(name, message):
    # Searches for friend on Google Hangouts and messages them
    print("Make sure the Google Hangout 'Conversations' page is visible and your cursor is not currently on the page.")
    time.sleep(3)
    ...
    while attempts > 0:
        ...
        if pyautogui.locateOnScreen('message.png') is None:
            ...
            if attempt == 1:
                ...
                print('Sending message to %s failed. Another {} attempts will be made before moving on.'.format(name, attempts))
            attempts -= 1
